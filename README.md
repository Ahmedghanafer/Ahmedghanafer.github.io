...
- Every animation is behind `prefers-reduced-motion`, and the *revealed* state is the CSS default —
  if `IntersectionObserver` never fires, the content is simply there. A failsafe in `site.js`
